...
def init_db():
    ...
    # Verifica se deve inserir professores padrão
    cursor.execute("SELECT COUNT(*) FROM professores")
    if cursor.fetchone()[0] == 0:
        cursor.execute("INSERT INTO professores (nome, materia) VALUES (?, ?)", ("REBECA CARVALHO", "Língua Portuguesa"))
        cursor.execute("INSERT INTO professores (nome, materia) VALUES (?, ?)", ("Fernanda Lima", "Matemática"))
...
def get_professores():
    conn = get_db_connection()
    df = pd.read_sql_query("SELECT * FROM professores", conn)
    conn.close()
    return df
...
# --- ABA 2: EDITAR ALUNOS ---
    ...
    if not df_alunos.empty:
        ...
        # Cria identificador único (Nome + Turma) para evitar conflitos de nomes iguais
...
# --- ABA 3: CADASTRO DE PROFESSORES ---
    ...
    st.markdown("Registre novos professores e suas respectivas matérias. Os dados são mantidos permanentemente.")
    ...
    with col_form_prof:
        ...
        if submeter_prof:
            if not nome_prof.strip():
                ...
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("INSERT INTO professores (nome, materia) VALUES (?, ?)", (nome_prof.strip(), materia_prof.strip()))
                conn.commit()
                conn.close()
                st.success(f"🎉 Professor(a) **{nome_prof}** cadastrado(a) com sucesso!")
                st.rerun()

    with col_list_prof:
        st.subheader("📋 Professores Cadastrados")
        df_prof = get_professores()
        if not df_prof.empty:
            # Exibe tabela limpa
            st.dataframe(df_prof[["nome", "materia"]].rename(columns={"nome": "Nome", "materia": "Matéria"}), use_container_width=True)
            
            # Excluir professores
            st.markdown("### 🗑️ Remover Professor")
            excluir_lista = df_prof["nome"].tolist()
            prof_a_excluir = st.selectbox("Escolha um professor para remover:", excluir_lista)
            if st.button("Remover Professor Selecionado"):
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM professores WHERE nome=?", (prof_a_excluir,))
                conn.commit()
                conn.close()
                st.success(f"Professor **{prof_a_excluir}** removido.")
                st.rerun()
        else:
            st.info("Nenhum professor cadastrado.")

# --- ABA 4: VISUALIZAR ALUNO EM SEPARADO (BOLETIM) ---
