...
if menu == "📋 Cadastrar Aluno":
    ...
    Se você preencher apenas duas notas e deixar as outras como zero, a média será calculada dividindo por 2.
    """)
...
# --- ABA 2: EDITAR ALUNO (FORMULÁRIO) ---
    ...
    st.markdown("Selecione um aluno específico na caixa abaixo para editar todas as suas notas, nome ou turma no Banco de Dados.")
    ...
    if not df_alunos.empty:
        ...
        if excluir_aluno:
            ...
            conn.commit()
            conn.close()
            st.success(f"🗑️ Aluno **{aluno_dados['nome']}** foi excluído com sucesso do Banco de Dados!")
            st.rerun()
...
# --- ABA 3: EDIÇÃO RÁPIDA (PLANILHA) ---
    ...
    st.markdown("""
    Altere as notas ou dados de **múltiplos alunos ao mesmo tempo** direto na planilha abaixo!
    
    👉 **Instruções:**
    1. Dê um **duplo clique** na célula que deseja alterar (Nome, Turma, ou Notas).
    2. Modifique o valor.
    3. Clique no botão de **Salvar Alterações da Tabela** no final para recalcular dinamicamente e salvar tudo de uma vez.
    """)
    ...
    if not df_alunos.empty:
        ...
        if st.button("💾 Salvar Alterações da Tabela"):
            ...
            conn.commit()
            conn.close()
            st.success("🎉 Todas as alterações feitas na planilha foram salvas com recálculo automático de médias!")
            st.rerun()
    else:
        st.info("Nenhum aluno cadastrado no momento. Acesse '📋 Cadastrar Aluno' para registrar os dados.")
...
# --- ABA 5: PAINEL GERAL (RESULTADOS) ---
    ...
    if not df_alunos.empty:
        ...
        st.markdown("### ⚙️ Opções de Exportação e Banco de Dados")
