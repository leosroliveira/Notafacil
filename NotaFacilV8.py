...
# --- FUNÇÃO DE CONVERSÃO PARA EXCEL ESTILIZADO (.XLSX) ---
def converter_para_excel_estilizado(df):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Importa ferramentas de estilo do openpyxl
        from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
        
        # Paleta de cores suaves (Lilás pastel profissional para combinar com o app da Rebeca)
        header_fill = PatternFill(start_color="A29BFE", end_color="A29BFE", fill_type="solid") 
        ...
        # Autoajuste automático de colunas
...
if menu == "📋 Cadastrar Aluno":
    ...
    st.markdown("""
    Insira o nome do aluno, selecione a turma (1º ao 9º Ano) e adicione as notas de Língua Portuguesa.
...
# --- ABA 5: PAINEL GERAL (RESULTADOS) ---
    ...
    st.markdown("Consulte os dados consolidados da classe, médias globais de Língua Portuguesa e faça exportações no formato Excel.")
    ...
    if not df_alunos.empty:
        ...
        # EXPORTAÇÃO EXCEL CUSTOMIZADA E CONTROLE
        st.markdown("---")
        st.markdown("### 📥 Opções de Exportação Avançadas (Excel .xlsx)")
        st.markdown("Selecione o formato de exportação desejado para gerar uma planilha editável do Excel:")
        ...
        if tipo_exportacao == "Planilha de Todos os Alunos (Toda a Escola)":
            with col_exp_1:
                ...
                excel_data = converter_para_excel_estilizado(df_export)
                st.download_button(
            ...
            if turmas_cadastradas:
                ...
                with col_exp_2:
                    ...
                    excel_data = converter_para_excel_estilizado(df_turma)
                    st.download_button(
            ...
            if lista_nomes:
                ...
                with col_exp_2:
                    ...
                    excel_data = converter_para_excel_estilizado(df_ind)
                    nome_aluno_limpo = df_ind["Nome do Aluno"].values[0].replace(" ", "_")
                    st.download_button(
                        label=f"📥 Baixar Excel do Aluno: {df_ind['Nome do Aluno'].values[0]} (.xlsx)",
                        data=excel_data,
